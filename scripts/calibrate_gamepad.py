...
def get_calibration_file_path(gamepad_type):
    ...
    gamepad_configs_dir = os.path.join(project_root, "gamepad_configs")
    # 确保 gamepad_configs 目录存在
    os.makedirs(gamepad_configs_dir, exist_ok=True)
...
def main():
    ...
    if save_calibration(calibration):
        ...
        print("现在可以在 feap_e3_mujoco_deploy.py 中使用校准后的手柄了")
    else:
